...
/// @dev router对调用函数时的参数做了一些校验，以避免在钱包合约中做校验从而增大钱包合约的尺寸
contract Router {
/// @dev 各个钱包实例的signer数量
...
/// @dev 初始化钱包时，钱包有效的signer数量在router中进行记录
        record[newWalletAddr]=signers.length;
        
        return newWalletAddr;
    }

/// @dev 钱包设置新Quorum时，新Quorum数量不能等于0,也不能大于当前signer的数量
/// @dev 合法性通过之后钱包合约仅需判断签名合法性即可
...
/// @dev 设置地址时可能会同时更新维护在本合约中的mapping及信任的signer地址数量
